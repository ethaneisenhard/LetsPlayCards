...
interface SnapState {
  ...
  /** Center pile; last element is the top. */
  center: Card[];
  /** Players who have run out (out for good in Snap). */
  out: string[];
  winner: string | null;
}
...
export const snapGame: CardGame = {
  type: 'snap',
  config: GAME_CONFIGS.snap,
  family: 'collecting',
  deck: {},
  setup(state) {
    ...
    return {
      game: {
        ...
        discardPile: [],
        currentSeat: 0,
        gameState: { stacks, center: [], out: [], winner: null } satisfies SnapState,
      },
  ...
  reduce(state, action) {
    const { game, players } = state;
    const gs = game.gameState as SnapState;
    ...
    if (action.intent === 'snap') {
      const player = findPlayer(players, action.playerId!);
      if (!player) throw new EngineError('Player not found');
      if ((stacks[player.id] ?? []).length === 0) throw new EngineError('You are out');
      if (center.length < 2) throw new EngineError('Not enough cards to snap');
      const matches = center[center.length - 1].rank === center[center.length - 2].rank;
      if (matches) {
        const won = center;
        center = [];
  ...
  isTerminal(state) {
    return !!(state.game.gameState as SnapState).winner;
  },
  score(state) {
    const gs = state.game.gameState as SnapState;
