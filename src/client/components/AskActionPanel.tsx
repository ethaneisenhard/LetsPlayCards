...
const chipPhone = `${chip} min-h-11 min-w-11 px-3 text-base`;
...
export function AskActionPanel({
  ...
  return (
    <div className={`flex flex-col items-center w-full ${phone ? 'gap-1 max-w-none px-0' : 'gap-2.5 max-w-sm px-2'}`}>
      ...
      {ranks && ranks.length > 0 && onSelectRank && (
        <div className={`flex flex-col items-center w-full ${phone ? 'gap-0' : 'gap-1.5'}`}>
          {!phone && <span className="text-white/40 text-[10px] uppercase tracking-widest">Number or face</span>}
          <div
            role="group"
            aria-label="Number or face"
            className={`flex flex-wrap justify-center ${phone ? 'gap-1' : 'gap-1.5'}`}
          >
            {ranks.map((rank) => (
              <button
                key={rank}
      ...
      {targets.length > 0 && (
        <div className={`flex flex-col items-center w-full ${phone ? 'gap-0' : 'gap-1.5'}`}>
          {!phone && <span className="text-white/40 text-[10px] uppercase tracking-widest">Who</span>}
          <div
            role="group"
            aria-label="Who"
            className={`flex flex-wrap justify-center ${phone ? 'gap-1' : 'gap-1.5'}`}
      ...
      <button
        ...
        className={`w-full rounded-xl bg-emerald-700 hover:bg-emerald-600 disabled:bg-white/10 disabled:text-white/35 disabled:cursor-not-allowed text-white font-bold shadow-lg ${
          phone ? 'max-w-none min-h-12 px-4 text-base' : 'max-w-xs px-4 py-2.5 text-sm'
        }`}
      >
        {submitLabel}
      </button>
